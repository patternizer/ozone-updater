% function read_extract_woudc_UOFT()

%--------------------------------------------------------------------------
% Called by load_woudc_UOFT.bat:
%
% C:\Users\patternizer\Downloads\wget64 --reject "robots.txt" --mirror --recursive --no-parent --reject "index.html*" 
% ftp://exp-studies.tor.ec.gc.ca/pub/ozonedata/Preliminary/
% matlab -nosplash -nodesktop -noFigureWindows -r "try; run('C:\Users\patternizer\Downloads\read_extract_woudc_UOFT.m'); catch; end; quit"
%--------------------------------------------------------------------------
% Dr. Michael Taylor: 
...
disp(['IN: read_extract_woudc_UOFT']);

%% PARAMETERS
dir_stations = 'C:\Users\patternizer\Downloads\exp-studies.tor.ec.gc.ca\pub\ozonedata\Preliminary\UOFT\';
...
for i = 1:nstations    
    ...
    for j = 1:ninstruments          
        ...
        year_list = str2double(year_array(1,3:end)); clear year_array;
        ...
        if isequal(exist(filename,'file'),0)
            ...
            year_list(year_list < year_last) = []; clear year_last
        end        
        ...
        for k=1:nyear
            ...
            idx = [idx,find(date_list(:) <= date_last)'];
            date_list(idx) = [];
            file_list(idx) = [];
...
disp(['OUT: read_extract_woudc_UOFT']);
